...
class CountryOverallCommand extends Command
{
    ...
    protected static $defaultName = 'app:taxes:country:overall';

    protected function configure()
    {
        $this
            // the short description shown while running "php bin/console list"
            ->setDescription('Output the overall amount of taxes collected per country')

            // the full command description shown when running the command with
            // the "--help" option
            ->setHelp('This command allows you to output the overall amount of taxes collected per country')
            ->addOption(
                'source',
                null,
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        )->calculateOverallTaxPerCountry(
            $input->getOption('country')
        );

        $output->writeln([
            '<info>The collected overall taxes of the country</info>',
            '<info>===============================================</info>',
            $result
        ]);
    }
