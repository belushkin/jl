...
class CountryAverageTaxRateCommand extends Command
{
    ...
    protected static $defaultName = 'app:taxes:country:rate:average';

    protected function configure()
    {
        $this
            // the short description shown while running "php bin/console list"
            ->setDescription('Output the average tax rate per country')

            // the full command description shown when running the command with
            // the "--help" option
            ->setHelp('This command allows you to output the average tax rate per country')
            ->addOption(
                'source',
                null,
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        )->calculateAverageTaxRatePerCountry(
            $input->getOption('country')
        );

        $output->writeln([
            '<info>The average tax rate of the country</info>',
            '<info>===============================================</info>',
            $result
        ]);
    }
