...
class StateOverallCommand extends Command
{
    ...
    protected static $defaultName = 'app:taxes:state:overall';

    protected function configure()
    {
        $this
            // the short description shown while running "php bin/console list"
            ->setDescription('Output the overall amount of taxes collected per state')

            // the full command description shown when running the command with
            // the "--help" option
            ->setHelp('This command allows you to output the overall amount of taxes collected per state')
            ->addOption(
                'source',
                null,
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        )->calculateOverallTaxPerState(
            $input->getOption('country'),
            $input->getOption('state')
        );
        $output->writeln([
            '<info>The overall amount of taxes collected per state</info>',
            '<info>===============================================</info>',
            $result
        ]);
    }
